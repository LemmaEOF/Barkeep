...
public record Drink(TextColor color, float colorStrength, int proof, List<FlavorNote> flavorNotes, List<StatusEffectInstance> effects) {
	public static final Codec<Drink> CODEC = RecordCodecBuilder.create(instance -> instance.group(
			...
			StatusEffectInstance.CODEC.listOf().fieldOf("effects").orElse(List.of()).forGetter(Drink::effects)
	...
	public String getTranslationKey(DynamicRegistryManager manager) {
		return translationKey(getId(manager));
	}

	public static String getTranslationKey(RegistryEntry<Drink> entry) {
		return translationKey(entry.getKey().get().getValue());
	}

	//TODO: here if I need to do anything like potion hardcoding, for now I don't need to
	private static String translationKey(Identifier id) {
		return id.toTranslationKey("drink");
	}
}
